...
def test_get_feed_empty_returns_fallback(client):
    response = client.get("/api/v1/feed?user_id=test-user")
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def test_reel_like_increases_count(client, db):
    ...
    response = client.post(f"/api/v1/reels/{reel.id}/like?user_id=test-user")
    assert response.status_code == 200
    data = response.json()
...
def test_reel_like_toggle_decreases_count(client, db):
    ...
    client.post(f"/api/v1/reels/{reel.id}/like?user_id=test-user")
    response = client.post(f"/api/v1/reels/{reel.id}/like?user_id=test-user")
