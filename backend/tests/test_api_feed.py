...
def test_get_feed_empty_returns_empty_list(client):
    response = client.get("/api/v1/feed?user_id=new-user-123")
...
def test_get_feed_falls_back_to_latest_reels(client, db):
    ...
    response = client.get("/api/v1/feed?user_id=test-user-fallback")
    assert response.status_code == 200
    reels = response.json()
    assert len(reels) >= 1
...
def test_get_feed_respects_limit(client, db):
    ...
    response = client.get("/api/v1/feed?user_id=test-user&limit=3")
    assert response.status_code == 200
...
def test_mark_consumed_updates_queue(client, db):
    ...
    response = client.post(f"/api/v1/feed/consumed/{reel.id}?user_id={user_id}")
    assert response.status_code == 200
    data = response.json()
    assert data["ok"] == True
...
def test_mark_consumed_returns_remaining_count(client, db):
    ...
    response = client.post(f"/api/v1/feed/consumed/{reels[0].id}?user_id={user_id}")
    assert response.status_code == 200
...
def test_feed_shows_queued_reels_first(client, db):
    ...
    response = client.get(f"/api/v1/feed?user_id={user_id}&limit=10")
    assert response.status_code == 200
    reels = response.json()
    assert len(reels) >= 1
...
def test_refill_endpoint_returns_status(client):
    response = client.post("/api/v1/feed/refill?user_id=refill-test-user")
    assert response.status_code == 200
    data = response.json()
...
def test_refill_needed_when_queue_empty(client):
    response = client.post("/api/v1/feed/refill?user_id=empty-queue-user")
    assert response.status_code == 200
