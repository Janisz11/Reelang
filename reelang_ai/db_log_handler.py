...
INSERT_APP_LOG = text(
    """
    INSERT INTO app_logs (level, logger_name, message, context)
    VALUES (:level, :logger_name, :message, CAST(:context AS jsonb))
    """
)
...
def _extract_context(record: logging.LogRecord) -> Optional[str]:
...
class DatabaseLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            try:
                session.execute(INSERT_APP_LOG, params)
                session.commit()
            finally:
                session.close()
        except Exception:
